...
int mcman_open2(int port, int slot, const char *filename, int flags)
{
	...
	if ((flags & (sceMcFileCreateFile | sceMcFileCreateDir)) != 0)
		printf("vmcman: open2 request port=%d slot=%d name='%s' flags=0x%x\n", port, slot, filename, flags);
#endif
...
#if defined(BUILDING_VMCMAN)
	if ((flags & (sceMcFileCreateFile | sceMcFileCreateDir)) != 0)
		printf("vmcman: open2 cachedirentry ret=%d found=%d cluster=%d fsindex=%d maxent=%d\n", r, fse1 != NULL, cacheDir.cluster, cacheDir.fsindex, cacheDir.maxent);
#endif
	if (r < 0)
		return r;
...
#if defined(BUILDING_VMCMAN)
	if ((flags & (sceMcFileCreateFile | sceMcFileCreateDir)) != 0)
		printf("vmcman: open2 parent read ret=%d parent_cluster=%d parent_fsindex=%d\n", r, fh->parent_cluster, fh->parent_fsindex);
#endif
	if (r != sceMcResSucceed)
		return r;
	...
	if (mcman_dircache[2].length == (u32)(cacheDir.maxent)) {
		...
		if ((flags & sceMcFileCreateDir) != 0)
			printf("vmcman: open2 create-dir flush ret=%d\n", r);
#endif
		if (r != sceMcResSucceed)
			return r;
...
#if defined(BUILDING_VMCMAN)
		if ((flags & sceMcFileCreateDir) != 0)
			printf("vmcman: open2 findfree-dir ret=%d\n", r);
#endif
		if (r < 0)
			return r;
		mcfree = r;
...
#if defined(BUILDING_VMCMAN)
	if ((flags & (sceMcFileCreateFile | sceMcFileCreateDir)) != 0)
		printf("vmcman: open2 target read ret=%d dir_cluster=%d maxent=%d\n", r, mcman_dircache[2].cluster, cacheDir.maxent);
#endif
	if (r != sceMcResSucceed)
		return r;
...
#if defined(BUILDING_VMCMAN)
		printf("vmcman: open2 create-dir parent_cluster=%d maxent=%d mcfree=%d name='%s'\n", mcman_dircache[2].cluster, cacheDir.maxent, mcfree, p);
#endif
		r = McCreateDirentry(port, slot, mcman_dircache[2].cluster, cacheDir.maxent, mcfree, (sceMcStDateTime *)&fse2->created);
#if defined(BUILDING_VMCMAN)
		printf("vmcman: open2 create-dir ret=%d\n", r);
#endif
		if (r != sceMcResSucceed)
			return -46;
